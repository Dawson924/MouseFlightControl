...
def load_data(file: str, namespace: str, default: Dict[str, Any]):
    if path.exists(file):
...
def save_data(file: str, configs: Dict[str, Dict]) -> None:
    parser = configparser.ConfigParser()
...
def validate_config(config: Dict[str, Any], silent: bool = False) -> Dict[str, Any]:
    ...
    for key, value in config.items():
        if key not in CONFIGURABLE:
            if not silent:
                raise ValueError(f"Invalid value: '{key}={value}' at {SCRIPT_PATH} ({key} does not exist)")
            continue
        ...
        if is_tuple and len(rule) >= 3:
            ...
            if max_val is not None:
                valid &= value <= max_val
            if not valid:
                raise ValueError(f"Invalid value: '{key}={value}' at {SCRIPT_PATH} (out of range: {min_val}~{max_val})")
